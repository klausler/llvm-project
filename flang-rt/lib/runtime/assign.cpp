...
RT_API_ATTRS int AssignTicket::Continue(WorkQueue &workQueue) {
  ...
  if (toDerived_) {
    if (int status{workQueue.BeginDerivedAssign(
  ...
  if (std::size_t numProcPtrs{procPtrDesc.Elements()}) {
    ...
    for (std::size_t k{0}; k < numProcPtrs; ++k) {
      ...
      for (; !Elementwise::IsComplete(); Elementwise::Advance()) {
        memmoveFct_(
            instance_.ElementComponent<char>(subscripts_, procPtr.offset),
            from_->ElementComponent<const char>(
                fromSubscripts_, procPtr.offset),
...
RT_API_ATTRS int DerivedAssignTicket::Continue(WorkQueue &workQueue) {
  // DerivedAssignTicket inherits from ElementComponentBase so that it
  // loops over elements at the outer level and over components at the inner.
  // This yield less surprising behavior for codes that care due to the use
  // of defined assignments when the ordering of their calls matters.
  while (!IsComplete()) {
    ...
    case typeInfo::Component::Genre::Automatic: {
      ...
      if (toDesc->IsAllocatable() && !fromDesc->IsAllocated()) {
        ...
        int nestedFlags{flags_ | DeallocateLHS};
        Advance();
        if (int status{workQueue.BeginAssign(
